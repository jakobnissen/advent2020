fn main() {
    ...
    println!("{}", part1(&instructions).1);
    println!("{}", part2(&mut instructions));
}
...
fn parse_instructions(path: &str) -> Vec<Command> {
    std::fs::read_to_string(path).unwrap().trim().lines()
    .map(|line| line.split(' ')).map(|mut fields| {
...
fn part1(instructions: &[Command]) -> (bool, i32) {
    ...
    loop {
        ...
        match instructions[pointer] {
            ...
            Command(Instruction::Acc, x) => {acc += x as i32; pointer += 1}
        }        
    }
}

fn part2(instructions: &mut [Command]) -> i32 {
    for i in 0..instructions.len() {
        ...
        let (terminated, acc) = part1(instructions);
